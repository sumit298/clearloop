...
export function CommandPalette({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  const router = useRouter(); const inputRef = useRef<HTMLInputElement>(null); const [query, setQuery] = useState("");
  ...
  if (!open) return null;
  return <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/30 p-4 pt-[12vh] backdrop-blur-sm" onMouseDown={() => onOpenChange(false)}><div role="dialog" aria-modal="true" aria-label="Search or jump to" className="w-full max-w-xl overflow-hidden rounded-lg border border-border bg-[var(--popover)] shadow-2xl" onMouseDown={(event) => event.stopPropagation()}><div className="flex items-center gap-2 border-b border-border px-3"><Search className="size-4 text-text-muted" /><input ref={inputRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search or jump to…" className="h-12 min-w-0 flex-1 bg-transparent text-[14px] outline-none placeholder:text-text-muted" /><kbd className="rounded border border-border px-1.5 py-0.5 font-mono text-[10px] text-text-muted">ESC</kbd></div>{!query && <div className="px-3 pt-3 text-[10px] font-semibold uppercase tracking-widest text-text-muted">Navigation</div>}<div className="max-h-[55vh] overflow-y-auto p-2">{query === "" && <button onClick={() => go("/dashboard/features?create=1")} className="flex w-full items-center gap-3 rounded-md px-2.5 py-2 text-left hover:bg-[var(--surface-raised)]"><span className="flex size-7 items-center justify-center rounded border border-border"><Plus className="size-3.5 text-primary" /></span><span className="flex-1 text-[13px]">New feature</span><span className="text-[11px] text-text-muted">Create</span></button>}{items.map((item) => { const Icon = item.icon; return <button key={`${item.href}-${item.label}`} onClick={() => go(item.href)} className="flex w-full items-center gap-3 rounded-md px-2.5 py-2 text-left hover:bg-[var(--surface-raised)]"><Icon className="size-4 text-primary" /><span className="min-w-0 flex-1"><span className="block truncate text-[13px]">{item.label}</span><span className="block truncate text-[11px] text-text-muted">{item.detail}</span></span></button>; })}{items.length === 0 && <div className="px-3 py-10 text-center text-[13px] text-text-muted">No matching features, bugs, projects, pull requests, or releases.</div>}</div><div className="flex items-center gap-3 border-t border-border px-3 py-2 text-[10px] text-text-muted"><span><kbd className="rounded border border-border px-1">↵</kbd> open</span><span><kbd className="rounded border border-border px-1">⌘K</kbd> toggle</span><span className="ml-auto flex items-center gap-1"><Rocket className="size-3" />ClearLoop</span></div></div></div>;
}
